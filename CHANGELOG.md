...
- Prompted by Discord feedback (uaRO player "Defectivve") on unclear prerequisites, Git/Xcode confusion, and a request for a gentler pace:
  ...
  - `README.md`: Option A's "pick any folder" (unexplained, per a cold-read agent flagging it as a genuine stumble) now gives a concrete default — pick Documents — confirmed against both desktop apps' actual new-chat flow instead of guessed.
