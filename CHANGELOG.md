...
- `README.md`'s "Updating an existing install" section pointed at "your AI session" without saying how to open one — now explicitly points back at "either option above," so it stays correct now that there are two entry paths instead of one.
- `SKILL.md` Step 1 (Homebrew) now warns that macOS's own "Install Command Line Developer Tools" dialog (triggered by `git`/`cc`, which Homebrew depends on) can pop up on a genuinely fresh Mac — the actual root cause of Defectivve's "hardest part" comment. Tells the AI to explain the popup in plain language before the user sees it unexplained, and gives an `xcode-select -p` check for confirming it actually finished if it was dismissed instead of installed.
